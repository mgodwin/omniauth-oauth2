  ...
  module Strategies
    ...
    class OAuth2
      ...
      def authorize_params # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
        ...
        session["omniauth.states"].shift if session["omniauth.states"].size > 50 # Prevent cookie overflow if states pile up.
      ...
      def callback_phase # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/MethodLength, Metrics/PerceivedComplexity
        ...
        valid_states = session["omniauth.states"] || []
        if error
          fail!(error, CallbackError.new(request.params["error"], request.params["error_description"] || request.params["error_reason"], request.params["error_uri"]))
        elsif !options.provider_ignores_state && (state.empty? || !valid_states.include?(state))
          fail!(:csrf_detected, CallbackError.new(:csrf_detected, "CSRF detected"))
        else
          session['omniauth.states']&.delete(state)
